...
func TradeValidate(client *gosf.Client, request *gosf.Request) *gosf.Message {
	...
	sender := models.GetPlayer(d.User.ID)
	receiver := models.GetPlayer(d.OtherID)
	...
	if !sender.VerifyChars(content.UserSends.Characters) || !sender.VerifyGrimms(content.UserSends.Grimms) {
	...
	if !receiver.VerifyChars(content.TargetSends.Characters) || !receiver.VerifyGrimms(content.TargetSends.Grimms) {
		return gosf.NewFailureMessage("There is an issue with your target's personas.")
	}

	if sender.TradeSent >= 5 {
		return gosf.NewFailureMessage("You have too much trade on hold.")
	}
	if receiver.TradeReceived >= 8 {
		return gosf.NewFailureMessage("This person already has too much trades on hold.")
	}

	PlayerResult := &TradeValidateContent{
		Money:          int64(sender.TotalBalance()) + content.TargetSends.Money - content.UserSends.Money,
		Boxes:          int64(sender.Boxes.Boxes) + content.TargetSends.Boxes - content.UserSends.Boxes,
		RareBoxes:      int64(sender.Boxes.RareBoxes) + content.TargetSends.RareBoxes - content.UserSends.RareBoxes,
		GrimmBoxes:     int64(sender.Boxes.GrimmBoxes) + content.TargetSends.GrimmBoxes - content.UserSends.GrimmBoxes,
		RareGrimmBoxes: int64(sender.Boxes.RareGrimmBoxes) + content.TargetSends.RareGrimmBoxes - content.UserSends.RareGrimmBoxes,
	}

	TargetResult := &TradeValidateContent{
		Money:          int64(receiver.TotalBalance()) - content.TargetSends.Money + content.UserSends.Money,
		Boxes:          int64(receiver.Boxes.Boxes) - content.TargetSends.Boxes + content.UserSends.Boxes,
		RareBoxes:      int64(receiver.Boxes.RareBoxes) - content.TargetSends.RareBoxes + content.UserSends.RareBoxes,
		GrimmBoxes:     int64(receiver.Boxes.GrimmBoxes) - content.TargetSends.GrimmBoxes + content.UserSends.GrimmBoxes,
		RareGrimmBoxes: int64(receiver.Boxes.RareGrimmBoxes) - content.TargetSends.RareGrimmBoxes + content.UserSends.RareGrimmBoxes,
	}
	...
	if len(sender.Grimms)+len(content.TargetSends.Grimms)-len(content.UserSends.Grimms) < 0 {
	...
	if len(receiver.Grimms)-len(content.TargetSends.Grimms)+len(content.UserSends.Grimms) < 0 {
		return gosf.NewFailureMessage("They would be left without any grimms.")
	}

	s := discord.Session
	SenderUser, _ := s.User(sender.DiscordID)
	SenderDM, _ := s.UserChannelCreate(sender.DiscordID)
	ReceiverUser, _ := s.User(receiver.DiscordID)
	ReceiverDM, _ := s.UserChannelCreate(receiver.DiscordID)

	SenderMessage := content.UserSends.PrepareMessage(sender)
	ReceiverMessage := content.TargetSends.PrepareMessage(receiver)
	...
	t := models.Trade{
		...
		UserSends:   content.UserSends.ToModel(TradeID, sender.DiscordID, "sender"),
		TargetSends: content.TargetSends.ToModel(TradeID, receiver.DiscordID, "receiver"),
	}
...
func (c *TradeValidateContent) PrepareMessage(p *models.Player) string {
	Message := ""
	if c.Money > 0 {
		Message += fmt.Sprintf("Money : **%d**Ⱡ.\n", c.Money)
	}
	if c.Boxes > 0 {
		Message += fmt.Sprintf("Boxes : **%d**.\n", c.Boxes)
	}
	if c.GrimmBoxes > 0 {
		Message += fmt.Sprintf("Grimm Boxes : **%d**.\n", c.GrimmBoxes)
	}
	if c.RareBoxes > 0 {
		Message += fmt.Sprintf("Rare Boxes : **%d**.\n", c.RareBoxes)
	}
	if c.RareGrimmBoxes > 0 {
		Message += fmt.Sprintf("Rare Grimm Boxes : **%d**.\n", c.RareGrimmBoxes)
	}
